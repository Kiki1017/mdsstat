...
test_that("outputs are as expected", {
  expect_equal(a1$test_name, "x-bar Western Electric Rule 1")
...
test_that("df parameter functions as expected", {
  ...
  expect_true(all(names(a2) %in% c("test_name",
                                   ...
                                   "params",
                                   "data")))
  expect_equal(a2$test_name, "x-bar Western Electric Rule 1")
...
a2 <- xbar(a2d, ts_event=c("Rate"="rate"))
...
a2 <- xbar(a2d, analysis_of="Testing")
test_that("ts_event parameter functions as expected", {
  expect_equal(a2$analysis_of, "Testing")
})

a2 <- xbar(a2d, eval_period=3)
test_that("eval_period parameter functions as expected", {
  ...
  expect_equal(names(xbar(data, eval_period=2)$status),
               ">3 time periods required")
  expect_error(xbar(a2d, eval_period=0))
  expect_error(xbar(a2d, eval_period=nrow(a2d) + 1))
})

test_that("zero_rate parameter functions as expected", {
  expect_is(xbar(a2d, zero_rate=0), "mdsstat_test")
  expect_is(xbar(a2d, zero_rate=1), "mdsstat_test")
  expect_error(xbar(a2d, zero_rate=1.1))
  expect_error(xbar(a2d, zero_rate=-1))
})

a22L <- xbar(a2d, we_rule=2)
a23L <- xbar(a2d, we_rule=3)
a24L <- xbar(a2d, we_rule=4)
test_that("we_rule parameter functions as expected", {
  ...
  expect_error(xbar(a2d, we_rule=1.1))
  expect_error(xbar(a2d, we_rule=5))
  expect_error(xbar(a2d, we_rule=5))
  expect_error(xbar(a2d, we_rule=0))
})
test_that("we_rule 2 functions as expected", {
  expect_equal(a22L$test_name, "x-bar Western Electric Rule 2")
...
test_that("we_rule 3 functions as expected", {
  expect_equal(a23L$test_name, "x-bar Western Electric Rule 3")
...
data <- data.frame(time=c(1:25), event=as.integer(stats::rnorm(25, 100, 25)))
a14L <- xbar(data, we_rule=4)
test_that("we_rule 4 functions as expected", {
  expect_equal(a14L$test_name, "x-bar Western Electric Rule 4")
